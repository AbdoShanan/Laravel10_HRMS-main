...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('employees', function (Blueprint $table) {
            ...
            $table->string("emp_national_idenity", 50)->comment("رقم البطاقة الشخصية - او رقم الهوية");
            ...
            $table->string('blood_group_id')->nullable()->comment("فصيلة الدم");
            $table->string('religion_id')->nullable()->comment(" الديانة");
            // $table->foreignId('religion_id')->comment('حقل الديانة')->references('id')->on('religions')->onUpdate('cascade');
            $table->string("emp_lang_id")->nullable()->comment(" اللغه التي يتكلم بها الاساسية");
            $table->string("emp_email", 100)->comment(" ايميل  الموظف");
            $table->string("country_id")->nullable()->comment("دولة الموظف");
            $table->string("governorate_id")->nullable()->comment("محافظة الموظف");
            $table->string("city_id")->nullable()->comment("مدينة الموظف");
            ...
            $table->string('driving_license_types_id')->nullable();
            $table->tinyInteger("has_Relatives")->default(2)->comment("هل له اقارب بالعمل ");
            $table->string("Relatives_details", 600)->nullable()->comment("تفاصيل الاقارب بالعمل");
            $table->string("notes",600)->nullable();
            $table->date("emp_start_date")->comment("تاريخ بدء العمل للموظف");
            $table->tinyInteger("Functiona_status")->default(0)->comment("حالة الموظف واحد يعمل - صفر خارج الخدمة");
            $table->string("emp_Departments_code");
            $table->string("emp_jobs_id");
            ...
            $table->string("emp_social_status_id")->nullable()->comment("الحالة الاجتماعية");
            ...
            $table->string("emp_nationality_id")->comment(" جنسية الموظف");
